package com.labella.store.order.web.rest;
...
import com.labella.store.order.config.KafkaProperties;
...
class LabellaStoreAppOrderKafkaResourceIT {
    ...
    void setup() {
        ...
        LabellaStoreAppOrderKafkaResource kafkaResource = new LabellaStoreAppOrderKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/labella-store-app-order-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/labella-store-app-order-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
